...
export default function EditHouse() {
    ...
    try {
      ...
      if (res.data.success) {
        ...
        setFormData({
          ...
          houseSize: house.houseSize || "", // Changed from roomSize
          availableFor: house.availableFor || [],
          quantity: house.quantity || 1,
          availability: house.availability !== undefined ? house.availability : true,
          isVerified: house.isVerified !== undefined ? house.isVerified : true,
          isShow: house.isShow !== undefined ? house.isShow : false,
          
          facilities: house.facilities || [],
          pricePlans: house.pricePlans || [],
          
          mainImage: house.mainImage || null,
          otherImages: house.otherImages || [],
          youtubeLink: house.youtubeLink || "",
          
          description: house.description || "",
          horooDescription: house.horooDescription || ""
        });

        setNearbyAreas(house.nearbyAreas?.length ? house.nearbyAreas : [""]);
      }
    } catch (error) {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      const res = await adminApiCall(`/house/edit/${houseId}`, {
        method: 'PUT',
    ...
    <div className="min-h-screen ml-72 bg-gray-50 p-6">
      ...
      <div className="bg-white rounded-lg shadow-sm p-6 mb-6">
        <div className="flex items-center justify-between">
          <div className="flex items-center gap-3">
            ...
            <div>
              <h1 className="text-2xl font-bold text-gray-800">Edit House</h1>
              <p className="text-gray-600">Update house property details</p>
            </div>
          </div>
        </div>
      </div>
      ...
      <form onSubmit={handleSubmit} className="space-y-6">
        ...
        <div className="bg-white rounded-lg shadow-sm p-6">
          <div className="flex items-center gap-2 mb-6">
            <FaHome className="text-orange-500" />
            <h2 className="text-xl font-semibold text-gray-800">House Details</h2>
          </div>
